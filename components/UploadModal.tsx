...
function UploadModal() {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async(values) => {
        try {
            ...
            //Upload image
            const { data: imageData, error: imageError } = await supabaseClient.storage.from('image').upload(`image-${values.title}-${uniqueID}`, imageFile, { cacheControl: '3600', upsert: false });
